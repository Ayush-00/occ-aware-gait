...
class GaitGL_occ_aware_2Lin(BaseModel):
    """
    ...
    def __init__(self, *args, **kargs):
        super(GaitGL_occ_aware_2Lin, self).__init__(*args, **kargs)
    ...
    def build_network(self, model_cfg):
        ...
        self.occ_mixer_fc = nn.Sequential(
            SeparateFCs(64, 64 + in_c[-1], 64+in_c[-1]),     #(parts_num, original_gaitgl embed size, desired size of occlusion aware embedding)
            nn.LeakyReLU(inplace=True),
            SeparateFCs(64, 64 + in_c[-1], 64+in_c[-1])
        )
        if 'SeparateBNNecks' in model_cfg.keys():
            self.BNNecks = SeparateBNNecks(**model_cfg['SeparateBNNecks'])
            self.Bn_head = False
        else:
            self.Bn = nn.BatchNorm1d(64+in_c[-1])
            self.Head1 = SeparateFCs(64, 64+in_c[-1], class_num)         #(64+in_c because desired occlusion mixed size is twice of gaitgl normal embed size)
            self.Bn_head = True
            
            
    def init_parameters(self):
        super(GaitGL_occ_aware_2Lin, self).init_parameters()
    ...
    def forward(self, inputs):
        ...
        with torch.no_grad():
            ...
            occ_embed = occ_embed.unsqueeze(2).repeat(1,1,64)   #(batch, embed, p)  repeated
