...
class GaitGL_occ_aware_guided_add(BaseModel):
    """
    ...
    def __init__(self, *args, **kargs):
        super(GaitGL_occ_aware_guided_add, self).__init__(*args, **kargs)
    ...
    def build_network(self, model_cfg):
        ...
        #self.occ_mixer_fc = SeparateFCs(64, 64 + in_c[-1], 64+in_c[-1])     #(parts_num, original_gaitgl embed size, desired size of occlusion aware embedding)
        self.occ_fc = nn.Linear(occ_dim, in_c[0])     #(parts_num, original_gaitgl embed size, desired size of occlusion aware embedding)
    ...
    def init_parameters(self):
        super(GaitGL_occ_aware_guided_add, self).init_parameters()
    ...
    def forward(self, inputs):
        ...
        occ_embed = occ_embed.permute(0,2,1).contiguous()    #(batch, frames, channels)
        occ_embed = self.occ_fc(occ_embed)   #(batch, frames, new_channels)
        occ_embed = occ_embed.permute(0,2,1).contiguous()    #(batch, new_channels, frames)
        occ_embed = occ_embed.unsqueeze(-1).unsqueeze(-1).repeat(1,1,1,h_out,w_out)   #(batch, channels, frames, h, w) repeated 
        outs = outs + occ_embed   #(batch, channels, frames, h, w)
        #print(f"Added embeddings yaaay GREW")
        ...
        # gait = torch.cat([gait, occ_embed], dim=1)  #(n, c + occ_dim, p)
        # gait = self.occ_mixer_fc(gait)  #(n, 64+c, p)
